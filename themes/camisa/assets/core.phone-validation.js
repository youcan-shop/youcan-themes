if (!customElements.get("ui-phone-validation")) {
  class PhoneValidation extends HTMLElement {
    ...
    constructor() {
      ...
      this.phoneHiddenInput = this.querySelector('[ui-phone-validator="hidden-input"]');
    ...
    validateElements() {
      return this.phoneSelectCountryCode && this.phoneDisplayedCountryCode && this.phoneNumber && this.phoneHiddenInput;
    }
    ...
    syncCountryCodeFromInput() {
      ...
      try {
        ...
        if (parsed) {
          ...
          const matchingOption = Array.from(this.phoneSelectCountryCode.options).find((option) => option.value === parsed.countryCallingCode);
