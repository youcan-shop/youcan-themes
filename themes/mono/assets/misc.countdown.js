if (!customElements.get("ui-countdown")) {
  class Countdown extends HTMLElement {
    ...
    constructor() {
      super();
      const [days, hours, minutes, seconds] = this.querySelectorAll("[data-days], [data-hours], [data-minutes], [data-seconds]");
      this.days = days;
      this.hours = hours;
    ...
    _render() {
      ...
      const { paddedDate, paddedTime } = this.getPaddedDateTimeString(date, time);
    ...
    getPaddedDateTimeString(day, time) {
      const date = new Date();
      const paddedDate = `${date.getFullYear()}-${this.numberToPaddedString(date.getMonth() + 1)}-${this.numberToPaddedString(day)}`;
